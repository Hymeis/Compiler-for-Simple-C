...
public:
    ...
    void setIndirection(int indirection) { _indirection = indirection; }
    void setSpecifier(int specifier) { _specifier = specifier; }

    Type promote() {
        if (this->isArray()) { // T[] --> T*
            return Type(this->specifier(), this->indirection() + 1);
        }
        else if (this->specifier() == CHAR && this->indirection() == 0) { // CHAR --> INT
            return Type(INT, 0);
        }
        return *this; // no promotion
    }
    bool isPointer() const { return (_declarator == SCALAR && _indirection > 0) || _declarator == ARRAY;}
    bool isNumeric() const { return _declarator == SCALAR && _indirection == 0 && _specifier != VOID;}
    bool isPredicate() const { return isPointer() || isNumeric();}
    bool isCompatibleWith(const Type &that) const {
        if (this->isNumeric()) return that.isNumeric();
        if (this->isPointer() && that.isPointer()) {
            if ((_specifier == VOID && _indirection == 1) || (that.specifier() == VOID && that.indirection() == 1)) return true;
            return this->indirection() == that.indirection() && this->specifier() == that.specifier();
        }
        return false;
    }
};
