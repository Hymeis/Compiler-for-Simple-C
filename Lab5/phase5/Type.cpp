...
{
    if (_indirection > 0) return 8; // Pointers always have 8 bytes
    int size = 1; // CHAR as default -- 1 byte
    if (_specifier == INT) size = 4; // INT -- 4 byte
    else if (_specifier == LONG) size = 8; // LONG -- 8 byte
    assert(_specifier != VOID);
    // Array size = size of T * length
    // If not array, return size of T
    return this->isArray() ? size * _length : size; 
}